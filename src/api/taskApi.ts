...
export const useFetchTasks = (): FetchTasksResult => {
  const [posts, setPosts] = useState<FetchTasksResponse>({ data: [], meta: {} });
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const { removeTask, updateStoreTask, addTask } = useTaskStore();
  ...
  const deleteTask = async (id: number) => {
    setLoading(true);
    try {
      const res = await fetch(`https://cms.laurence.host/api/tasks/${id}`, { method: 'DELETE' });
