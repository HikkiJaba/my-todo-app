...
const items: MenuProps['items'] = ['Онлайн', 'Локально'].map((key) => ({
  key,
  label: `${key}`,
}));
...
function Sidebar({ onModeChange }: { onModeChange: (mode: 'Онлайн' | 'Локально') => void }) {
  const theme = useThemeStore((state) => state.theme);
  const setTheme = useThemeStore((state) => state.setTheme);
  const [selectedMode, setSelectedMode] = useState<'Онлайн' | 'Локально'>(
    (localStorage.getItem('mode') as 'Онлайн' | 'Локально') || 'Онлайн'
  );
    ...
    <SiderStyled width={250}>
      <SiderContainer>
        ...
        <SiderMenuStyled
          ...
          onSelect={({ key }) => setSelectedMode(key as 'Онлайн' | 'Локально')}
          items={items}
          theme="light"
        />
